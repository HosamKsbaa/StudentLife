...
ApiFutureButton({
  ...
  this.loading =
      const IconedButton(text: "جار التحميل", color: Colors.deepPurple),
  this.idle = const IconedButton(
      text: "اضف",

      icon: const Icon(Icons.add, color: Colors.cyan),
      color: Colors.white70),
  this.success = const IconedButton(
      text: "نجاح",
      icon: const Icon(Icons.check_circle, color: Colors.white),
      color: Colors.green),
  this.fail = const IconedButton(
      text: "فشل، أعد المحاولة",
...
class _ApiFutureButtonState<T> extends State<ApiFutureButton<T>> {
  final HDMHttpRequestsStates<T> hdmHttpRequestsStates =
      HDMHttpRequestsStates<T>();
